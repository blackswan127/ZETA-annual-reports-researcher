...
from .sources.dse_public import DSEPublic,shard_accept
...
class Pipeline:
    def __init__(self,work:Path,zeta_root:Path,cfg):self.work=Path(work);self.root=Path(zeta_root);self.work.mkdir(parents=True,exist_ok=True);self.db=DB(self.work/"harvest.sqlite3");self.http=Http(cfg.request_delay,cfg.timeout,cfg.max_retries);self.cfg=cfg
    def close(self):self.db.close()
    def universe(self,overrides=None,gleif=False,universe_csv=None,shard_count=1,shard_index=0):
        rows=[x for x in load_universe_csv(Path(universe_csv)) if shard_accept(x.ticker,shard_count,shard_index)] if universe_csv else DSEPublic(self.http).current_universe(shard_count,shard_index,True,self.cfg.discovery_workers);ovs=load_overrides(overrides) if overrides else {}
        for i in rows:
    ...
    def ingest_authorized(self,path):
        ...
        self.db.select_best()
    def discover_financial_links(self):
        src=DSEPublic(self.http);rows=self.db.conn.execute("SELECT * FROM issuers WHERE current=1 AND financials_url<>'' ORDER BY ticker").fetchall()
        with ThreadPoolExecutor(max_workers=self.cfg.discovery_workers) as ex:
            fut={ex.submit(src.financial_documents,r,self.cfg.start_year,self.cfg.end_year):r['ticker'] for r in rows}
            for f in as_completed(fut):
                t=fut[f]
                try:
                    for c in f.result():self.db.add_candidate(c)
                except Exception as e:self.db.event("WARN","dse_financial_link_failed",{"ticker":t,"error":str(e)})
        self.db.select_best()
    def discover_ir_missing(self):
        ...
        with ThreadPoolExecutor(max_workers=max(1,self.cfg.discovery_workers//2)) as ex:
            fut={ex.submit(discover_ir,self.http,r['ticker'],r['website'],self.cfg.start_year,self.cfg.end_year):r['ticker'] for r in rows}
            for f in as_completed(fut):
                t=fut[f]
                try:
                    for c in f.result():self.db.add_candidate(c)
    ...
    def download(self, tickers: list[str] | None = None, limit: int | None = None):
        ...
        with ThreadPoolExecutor(max_workers=self.cfg.download_workers) as ex:
            fut={ex.submit(fetch_candidate,self.http,r,self.root,self.work/"staging",self.cfg.min_pdf_bytes):r for r in rows}
            for f in as_completed(fut):
                r=fut[f]
                try:
