...
class MiddleEastPipeline:
    def __init__(self, config: RuntimeConfig):
        self.config = config
        self.work_dir = Path(config.work_dir)
        self.work_dir.mkdir(parents=True, exist_ok=True)
        self.db = MiddleEastDB(self.work_dir / "harvest.sqlite3")
    ...
    def stage_universe(
        ...
        issuers = load_universe(iso3=iso3, universe_csv=universe_csv, local_dir=self.work_dir.parent)
        if limit:
            issuers = issuers[:limit]
        for iss in issuers:
            self.db.upsert_issuer(iss)
            self.db.make_slots(iss.issuer_id, self.config.start_year, self.config.end_year, required_class=self.config.required_class)
        return issuers
    ...
    async def download(
        ...
        selected_rows = self.db.get_selected_candidates(repair=repair, required_class=self.config.required_class)
        if limit:
        ...
        async def download_one(row):
            ...
            final_target = build_final_path(self.output_root, iso3, mic, lei, isin, ticker, fy, lang="EN", report_type="AR")
            if final_target is not None:
                dest = final_target
                target_state = "DONE"
            else:
                dest = build_staging_path(self.work_dir / "staging", iso3, mic, ticker, fy, lang="EN", report_type="AR")
            ...
            if dest.exists():
                ...
                if valid:
                    ...
                    self.db.update_slot_status(iss_id, fy, target_state, selected_candidate_id=cand_id)
                    total_downloaded += 1
                    return
    ...
    async def run(
        ...
        issuers = self.stage_universe(iso3=iso3, universe_csv=universe_csv, limit=limit)
