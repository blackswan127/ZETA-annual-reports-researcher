...
class ArtifactDownloader:
    def __init__(self,workers=8,rps=4.0,timeout=60.0,retries=5,chunk_size=1024*1024,client=None):
        ...
        self.client=client or httpx.AsyncClient(timeout=httpx.Timeout(timeout,read=max(timeout,180)),follow_redirects=True,
    ...
    async def fetch(self,url:str,dest:Path,kind:str="PDF")->tuple[Path,int,str,int]:
        ...
        raw=dest.with_suffix(ext); part=raw.with_suffix(raw.suffix+'.part')
        async with self.sem:
            last=None
                ...
                try:
                    ...
                    async with self.client.stream('GET',url,headers=headers) as r:
                        if r.status_code==416 and offset:
                            if is_pdf(part) or is_zip(part): os.replace(part,raw); return self._finalize(raw,dest)
                            part.unlink(missing_ok=True); continue
                        if r.status_code==429 or 500<=r.status_code<600:
                            ra=r.headers.get('Retry-After'); delay=float(ra) if ra and ra.replace('.','',1).isdigit() else min(30,1.7**attempt)
                            await asyncio.sleep(delay+random.random()*.3); continue
                        r.raise_for_status()
                        ctype=r.headers.get('content-type','').lower()
                        if 'text/html' in ctype: raise RuntimeError('received HTML instead of document')
                        mode='ab' if offset and r.status_code==206 else 'wb'
                        with part.open(mode) as fh:
                            async for chunk in r.aiter_bytes(self.chunk_size):
                                if chunk: fh.write(chunk)
                    if not (is_pdf(part) or is_zip(part)): raise RuntimeError('download is neither PDF nor ZIP')
                    os.replace(part,raw); return self._finalize(raw,dest)
                except Exception as exc:
                    last=exc
                    if attempt+1<self.retries: await asyncio.sleep(min(30,1.7**attempt)+random.random()*.3)
