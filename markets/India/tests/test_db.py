...
def test_db_prefers_bse_ann(tmp_path):
    db = Database(tmp_path / 'x.db')
    i = Issuer('INE1', 'INE1', 'Test', nse_symbol='TEST', bse_scrip='500001', source_flags='BSE,NSE')
    db.replace_universe([i], {'deduped': 1})
    db.init_slots(2025, 2025)
    db.add_candidates([
        Candidate('INE1', 2025, 'NSE', 'n', 'Annual Report', 'http://n.pdf', score=500),
        Candidate('INE1', 2025, 'BSE_ANN', 'ann1', 'Annual Report', 'http://bse/AttachHis/a.pdf', score=200),
    ])
    db.select_best(2025, 2025)
    r = db.conn.execute('select c.source from expected_slots e join candidates c on c.id=e.selected_candidate_id').fetchone()
    assert r[0] == 'BSE_ANN'
    db.close()
